...
#### Error Feedback in Flexible Mode

The dashboard now surfaces errors directly in the chat panel rather than failing silently. You will see an orange warning message (with a `!` avatar) in the following situations:

- **Malformed AI response** — if Claude returns a JSON block that cannot be parsed, the chat will tell you the update was not applied and ask you to request a retry. Previously, this failed with no indication.
- **Truncated response** — if Claude's response was cut off before the JSON block closed, the chat will notify you explicitly.
- **Invalid transform fields** — before any transform is stored, the dashboard validates its required fields for all transform types:
  - *Combine*: checks that `expression` and `columns` are present, that all column names exist in your data, and that the expression has matching parentheses.
  - *Recode*: checks that `column` and `map` are present, and that the source column exists.
  - *Reverse score*: checks that `columns` and `max` are present, and that all source columns exist.
  - *Filter*: runs a dry test of the condition on your first data row and reports any syntax errors before applying.
  - *Lookup / Flag*: checks that `columns` and `labels` are present, have the same length, and that all source columns exist.
  - *Unsupported type*: if Claude proposes a transform type the platform cannot execute (e.g. a custom code block), you will be told which types are supported.
- **Runtime execution errors** — if a transform fails when Preview Data is run (e.g. a combine expression produces a runtime error), the chat reports the specific failure and indicates which transform caused it. The preview will show `null` for affected cells rather than a silent `ERR`.
- **Filter failures at confirmation** — if a filter condition fails when you click Confirm & Next Step, the chat warns you and skips the filter rather than crashing silently. You can go back and fix the condition before proceeding.

In all cases, the warning message describes what went wrong and suggests what to ask Claude to fix.
...
The dashboard has up to four tabs depending on your data.
...
- **Ask Claude** — an interactive chat panel below the interpretation (see [AI Chat](#ai-chat-ask-claude) below)
...
- **Ask Claude** — an interactive chat panel scoped to the selected participant's data
...
- **Ask Claude** — an interactive chat panel scoped to the current demographic grouping

#### Custom Explorer Tab

A flexible, free-form analysis page that lets you plot any variable in your dataset against any other. Always visible once data is loaded. See [Custom Explorer](#custom-explorer) below for full details.

---

### AI Chat — Ask Claude

Each dashboard tab includes an **Ask Claude** chat panel below the AI Interpretation section. This allows you to go beyond the auto-generated summary and have an open-ended conversation with Claude about the data currently in view.

#### How it works

The chat is context-aware: Claude is automatically given the data for the current tab (overall statistics, individual participant data, or group-level summaries) as background context. You do not need to paste any numbers — just ask your question in plain language.

Each tab maintains its own independent conversation history, so switching between tabs does not reset your previous questions.

#### What you can ask

You can ask any question about the data — descriptive, comparative, or theory-driven. Some examples:

- *"The primacy effect means the first day's bias should be the largest. Does this dataset show that pattern?"*
- *"Which day has the highest estimation error, and what might explain it?"*
- *"Does this participant's bias seem to increase or decrease over the study period?"*
- *"Is the gender difference in bias large enough to be practically meaningful?"*
- *"What might explain why this group underestimates while the other group overestimates?"*

You can also use the **Quick questions** chips at the top of the chat panel to send a pre-written question with one click.

#### Conversation controls

- **Enter** sends your message. **Shift+Enter** adds a new line.
- The **Clear** button resets the conversation history for the current tab.
- The chat retains the last 10 turns of context. For very long conversations, earlier messages are dropped automatically to avoid token limits.

#### Limitations

- The chat uses the data summaries visible on the current tab, not raw row-level data. Very granular questions (e.g. about a specific participant's exact value on a specific day) are better answered by switching to the Individual tab first.
- The chat requires the proxy server to be running. If there is no response, check your terminal.

---

### Custom Explorer

The **Custom Explorer** tab provides a free-form chart builder for exploring relationships between any variables in your dataset. Unlike the other tabs — which show fixed views of bias over time — the Explorer lets you choose your own X axis, Y axis, and color grouping.

#### Analysis Level

Before building a chart, select one of two analysis levels:

| Level | Unit of analysis | Best for |
|-------|-----------------|---------|
| **Person-level** | One point per participant (daily values averaged across all days) | Exploring trait-level predictors, e.g. does neuroticism predict average bias? |
| **Daily-level** | One point per participant-day observation | Exploring within-person variation, e.g. does daily well-being predict that day's bias? |

This distinction matters because some variables are measured once per person (e.g. a personality trait), while others are measured every day (e.g. daily mood). At person-level, all variables are reduced to a single value per participant before plotting. At daily-level, each row in the dataset becomes a data point.

#### Variable Pool

The X axis and Y axis dropdowns contain all variables that were not marked as **Ignored** during the data mapping step. These are organized into four groups:

- **Bias Metrics** — Screen Time Bias Error % and Pickups Bias Error % (computed automatically)
- **Core Variables** — subjective and objective time and pickups
- **Extension Variables** — any variables you classified as Extension during mapping (e.g. psychological scales, usage motivations)
- **Demographic Variables** — any variables you classified as Demographic (e.g. gender, age)

Variables marked as Ignored during mapping do not appear here.

Each variable is automatically labelled as **continuous** (8 or more unique values) or **categorical** (fewer than 8 unique values), and the chart type is chosen accordingly.

#### Chart Types

The chart type is determined automatically based on what you put on each axis:

| X axis type | Y axis type | Chart produced |
|-------------|-------------|----------------|
| Continuous | Continuous | Scatter plot |
| Categorical | Continuous | Bar chart |
| Continuous | Categorical | Not supported — swap the axes or choose a continuous Y variable |

If you add a **Color by** variable (always categorical), scatter plots show points in different colors per group, and bar charts become grouped bar charts.

#### OLS Trend Line

When the chart is a scatter plot, you can tick **Show OLS trend line** to overlay a simple linear regression line for each group. This helps you visually assess the direction and rough magnitude of the relationship. No statistical significance is reported — treat this as an exploratory aid, not a formal test.

#### Generating a Chart

1. Set the **Analysis Level**
2. Choose **X Axis** and **Y Axis** from the dropdowns
3. Optionally add a **Color by** variable
4. Click **Generate Chart**

The chart appears below, along with a summary of the configuration (number of observations, analysis level, chart type).

#### AI Interpretation

After generating a chart, click **Generate** in the AI Interpretation panel to get a 3–5 sentence plain-prose reading of the relationship shown. For scatter plots, Claude is given the correlation coefficient, OLS slope, and data range for each group. For bar charts, Claude is given the mean Y value per category.

The **Ask Claude** chat panel below lets you follow up with additional questions about the chart, just like on the other tabs.

#### Notes

- The Custom Explorer uses only the variables available after data mapping. If you expected a variable to appear but it is not in the dropdowns, check that it was not moved to **Ignored** during the mapping step.
- At person-level, continuous variables are averaged across all days for each participant. Categorical variables are assigned the most common value across days. This means person-level analysis discards within-person variation by design.
- At daily-level, person-level variables (e.g. a trait measured only once) will appear repeated across all days for the same participant. This is correct behavior — they are used as participant-level predictors of day-level outcomes.
- Re-clicking **Generate Chart** after changing settings will re-render the chart but will not reset the AI chat history.
